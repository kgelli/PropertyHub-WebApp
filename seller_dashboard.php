...
<div class="full">
  ...
  <section class="section-plans" id="section-plans">
    ...
    $pdo = new PDO('mysql:host=localhost;dbname=kgelli2', 'kgelli2', 'kgelli2');

    // query to retrieve the data from the database
    $sql = "SELECT * FROM cards WHERE seller_user_id='$username'";
    $stmt = $pdo->query($sql);
    echo '<div class="row">';
    ...
    echo '        <div class="card__cta">';
    echo '          <div class="card__price-box">';
    echo '            <h3><b>Add new property</b></h3>';
    echo '          </div>';
    echo '        </div>';
    echo '      </div>';
    echo '    </a>';  // Add this line to close the link
    echo '  </div>';
    echo '</div>';
    echo '</div>';
    ?>
    <section>
      <style>
        ...
        .card__details img {
          ...
          height: 100%;
          object-fit: cover;
          /* Ensures the image covers the entire container, maintaining aspect ratio */
        }
